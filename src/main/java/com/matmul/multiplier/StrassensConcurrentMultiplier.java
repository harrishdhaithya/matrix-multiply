...
public class StrassensConcurrentMultiplier implements Multiplier {
    ...
    public int[][] multiply(int[][] a, int[][] b) {
        return ForkJoinPool.commonPool().invoke(new StrassenTask(a, b));
    }
    ...
    private static class StrassenTask extends RecursiveTask<int[][]> {
        ...
        protected int[][] compute() {
            ...
            int[][] tmp1 = CommonUtils.matAdd(M1, M4);
            int[][] tmp2 = CommonUtils.matSub(tmp1, M5);
            int[][] C11  = CommonUtils.matAdd(tmp2, M7);

            int[][] C12 = CommonUtils.matAdd(M3, M5);
            int[][] C21 = CommonUtils.matAdd(M2, M4);

            int[][] tmp3 = CommonUtils.matAdd(M1, M3);
            int[][] tmp4 = CommonUtils.matSub(tmp3, M2);
            int[][] C22  = CommonUtils.matAdd(tmp4, M6);
